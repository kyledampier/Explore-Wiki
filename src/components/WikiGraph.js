...
function WikiGraph() {
  ...
  // initalize the first state of the program
  ...
  async function newSearchTermFound(event) {
    // this function is run when someone selects a new search term
    console.log(network);
    ...
    // wait until all propsises are resolved
    Promise.allSettled(requests).then((_) => {
      ...
      if (network) {
        ...
        // add list of nodes and edges generated by the data structure
        network.body.data.nodes.update(newRoot.getNodes());
        network.body.data.edges.update(newRoot.getEdges());
      }
      // update the root variable and refresh the function
      setRoot(newRoot);
    });
  };
  ...
  const events = {
    ...
    doubleClick: async function (event) {
      // this function runs when someone double clicks the graph
      var {nodes, edges} = event;

      // run DFS and get the result based on what node was selected
      let result = DFS(root, nodes[0]);
      if (result){
        let [requests, sortableArray] = await result.getChildren();

        // wait until all requests are settled
        Promise.allSettled(requests).then((_) => {
          sortableArray.sort((a,b) => b[0] - a[0]);
          if (!(result.children.length > 1)) {
            // recieve top links with shared categories
            for (var i = 0; i < numToKeep; i++)
              result.addChild(new WikiNode(sortableArray[i][1], getUrl(sortableArray[i][1]), sortableArray[i][1]), sortableArray[i][0]);
  
            if (network) {
              // update nodes and edges from generated list from WikiNode
